...
export default function groupPage() {
  const [groupTo, setGroupTo] = useState<IFirebaseGroup>(emptyGroup);
  const [groupFrom, setGroupFrom] = useState<IFirebaseGroup>(emptyGroup);
  const urlParams = useParams();
  const [isMatch, setIsMatch] = useState<boolean>();
  const [isGold] = useState<boolean>(true); //TODO
  const [match, setMatch] = useState<IFirebaseMatch>(emptyMatch);
